...
let state={tab:'climate',scale:'City',selectedPlanet:'Mercury',selectedHouse:1,profile:loadProfile(),engineStatus:'loading',engineMessage:'Loading Swiss Ephemeris…',swe:null,transit:null,latitude:36.17,longitude:-115.14,utcDateTime:defaultUTC,useLiveAsc:true,map:null,mapMarker:null,mapPointMarker:null,mapZoom:11,maptilerKey:localStorage.getItem('maptilerKey')||'',searchResults:[],searchStatus:'',selectedMapPoint:null};
...
function selectSearchResult(i){
  ...
  state.longitude=+r.center[0];state.latitude=+r.center[1];state.selectedMapPoint=null;
...
function bind(){
  ...
  const lat=document.querySelector('#latitude');if(lat)lat.onchange=()=>{state.latitude=+lat.value;moveMapToState()};
  const lon=document.querySelector('#longitude');if(lon)lon.onchange=()=>{state.longitude=+lon.value;moveMapToState()};
  ...
  const centerMap=document.querySelector('#centerMap');if(centerMap)centerMap.onclick=()=>{const a=document.querySelector('#latitude'),o=document.querySelector('#longitude');state.latitude=+a.value;state.longitude=+o.value;moveMapToState()};
