...
function selectSearchResult(i){
  ...
  state.longitude=+r.center[0];state.latitude=+r.center[1];state.selectedMapPoint=null;state.streetIndex=[];state.streetIndexStatus='';setTimeZoneFromCoords(true);
...
function bind(){
  ...
  const scale=document.querySelector('#scale');if(scale)scale.onchange=()=>{state.scale=scale.value;state.streetIndex=[];state.streetIndexStatus='';moveMapToState();render()};
